...
describe('PushHttpGetView (action tests)', () => {
  ...
  it('save calls config.saveAll when form is valid', async () => {
    ...
    const wrapper = mount(PushHttpGetView, { global: { plugins: [createTestingPinia()], stubs: { BsInputText: true, BsProgress: true, BsMessage: true } } })
  ...
  it('v-model bindings trigger config updates via emitting stubs', async () => {
    ...
    for (const el of wrapper.findAll('button')) { try { await el.trigger('click') } catch {} }
    expect(wrapper.find('form').exists()).toBe(true)
  })
})
