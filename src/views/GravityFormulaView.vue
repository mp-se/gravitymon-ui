            ...
            <div class="row">
                <div class="col-md-12">
                    <BsInputText v-model="config.gravity_formula" maxlength="200" label="Gravity formula"
                        help="Formula used to convert angle to gravity" :badge="badge.gravityFormulaBadge()"
                ...
                <template v-for="(data, index) in config.formula_calculation_data" :key="index">
                    <div class="col-md-6">
                        <div class="input-group has-validation">
                            ...
                            <input v-model="config.formula_calculation_data[index].a" class="form-control w-2"
                                type="number" min="0" max="90" step=".001"
                                :disabled="global.disabled || config.gyro_disabled">
                    ...
                    <div class="col-md-6">
                        <div class="input-group has-validation">
                            <span class="input-group-text">{{ index + 1 }}</span>
                            <input v-model="config.formula_calculation_data[index].g" class="form-control" type="number"
                                min="1" max="10" step=".0001" :disabled="global.disabled || config.gyro_disabled">
                            <span class="input-group-text">{{ config.gravity_format == 'G' ? 'SG' : 'P' }}</span>
                        </div>
                    </div>
                </template>

                <div class="form-text">Enter the data that is used to create a new formula. The most optimal formula
                    will be selected
                    and also validated towards these values.</div>
                ...
                <div class="col-md-6">
                    <BsInputNumber v-model="config.formula_max_deviation"
                        :unit="config.gravity_format == 'G' ? 'SG' : 'P'" label="Max allowed deviation" min="1" max="10"
                        step=".1" width="4"
                        ...
                        :disabled="global.disabled || config.gyro_disabled"></BsInputNumber>
                </div>
            </div>
